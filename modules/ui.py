...
def create_sampler_and_steps_selection(choices, tabname):
    if opts.samplers_in_dropdown:
        with FormRow(elem_id=f"sampler_selection_{tabname}"):
            ...
            steps = gr.Slider(minimum=1, maximum=500, step=1, elem_id=f"{tabname}_steps", label="Sampling steps", value=20)
    else:
                    ...
                    if category == "sampler":
                        ...
                        with FormGroup(visible=False, elem_id="txt2img_hires_fix") as hr_options:
                            with FormRow(elem_id="txt2img_hires_fix_row1", variant="compact"):
                                ...
                                hr_second_pass_steps = gr.Slider(minimum=0, maximum=500, step=1, label='Hires steps', value=0, elem_id="txt2img_hires_steps")
    ...
    with gr.Blocks(analytics_enabled=False) as img2img_interface:
        ...
        with FormRow().style(equal_height=False):
            with gr.Column(variant='compact', elem_id="img2img_settings"):
                ...
                with FormRow():
                    resize_mode = gr.Radio(label="Resize mode", elem_id="resize_mode", choices=["Just resize", "Crop and resize", "Resize and fill", "Just resize (latent upscale)", "Preserve size"], type="index", value="Just resize")
